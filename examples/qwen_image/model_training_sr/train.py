...
from .training_utils import DiffusionTrainingModule, ModelLogger, launch_training_task, qwen_image_parser
from diffsynth.extensions.realesrgan.dataset import PairedSROnlineTxtDataset
os.environ["TOKENIZERS_PARALLELISM"] = "false"


def replace_linear_with_duallora(model, patterns, rank, alpha1, alpha2, use_fp8=False, num_condition_images=1, noise_seq_len=None):
    """
    先冻结model所有参数，
...
class DualLoRALinear(torch.nn.Module):
    """
    对序列前一半噪声/后一半条件分别使用不同 LoRA 的 Linear 包装器。
    原 Linear 权重原地共享，不额外复制。
    """
    def __init__(self, linear, rank, alpha1, alpha2, noise_seq_len=None, num_condition_images=1):
        super().__init__()
        assert isinstance(linear, torch.nn.Linear)
        self.rank    = rank
        self.alpha1  = alpha1
    ...
    def forward(self, x):
        if x.ndim == 2:
            ...
            if self.noise_seq_len is not None:
                L = self.noise_seq_len
            else:
                total_parts = 1 + self.num_condition_images
                assert L2 % total_parts == 0, f"sequence length must be divisible by total parts {total_parts}"
                L = L2 // total_parts
...
class QwenImageTrainingModule(DiffusionTrainingModule):
    ...
    def add_custom_dual_lora(self, model, lora_rank, num_condition_images=1):
        patterns = [
            "img_in",
            "img_mod.1",
            "attn.to_q",
