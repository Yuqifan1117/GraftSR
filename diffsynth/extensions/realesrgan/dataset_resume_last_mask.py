...
class PairedSROnlineTxtDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split=None, args=None):
        ...
        if split == 'train':
            ...
            # 记录哪些样本有 ref（用于 __getitem__ 判断）
            self.has_ref = len(self.ref_list) == len(self.gt_list) and len(self.ref_list) > 0
            self.has_mask = len(self.hq_ori_mask_list) == len(self.gt_list) and len(self.hq_ori_mask_list) > 0
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            if self.has_mask and len(self.hq_ori_mask_list) > 0 and len(self.ref_crop_mask_list) > 0:
            ...
            # 加载参考图像
            if self.has_ref and len(self.ref_list) > 0:
                if use_hq:
