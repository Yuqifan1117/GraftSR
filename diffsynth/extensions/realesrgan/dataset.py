...
EDIT_WITH_REF_MASK_PROMPT_PREFIX = (
    "Enhance the low-quality image especially in the white-masked region to high resolution with sharp details and fine textures, "
    "using the high-quality reference image as a guide for texture and appearance: "
)
...
class PairedSROnlineTxtDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split=None, args=None):
        ...
        if split == 'train':
            ...
            if args.ref_txt_paths is not None:
                ...
                self.flexible_ref_resolution = False
            ...
            # 记录各子数据集大小，用于判断样本是否有真实 ref/mask
            self.sub_dataset_sizes = []
            for txt_path in args.dataset_txt_paths:
                with open(txt_path, 'r') as f:
                    self.sub_dataset_sizes.append(sum(1 for line in f if line.strip()))

            # ref/mask 可选加载，不再强制长度匹配
            self.ref_list = load_txt_paths(getattr(args, 'ref_txt_paths', None))
            self.hq_ori_mask_list = load_txt_paths(getattr(args, 'hq_ori_mask_txt_paths', None))
            self.ref_crop_mask_list = load_txt_paths(getattr(args, 'ref_crop_mask_txt_paths', None))

            # 显式指定哪些子数据集有 ref（逗号分隔索引，如 "0,2"）
            # 未指定时回退到旧逻辑：自动匹配最后一个子数据集
            ref_dataset_indices_str = getattr(args, 'ref_dataset_indices', None)
            if ref_dataset_indices_str is not None:
                self.ref_dataset_indices = set(int(x.strip()) for x in ref_dataset_indices_str.split(','))
            else:
                last_sub_size = self.sub_dataset_sizes[-1] if self.sub_dataset_sizes else 0
                if len(self.ref_list) == last_sub_size and last_sub_size > 0:
                    self.ref_dataset_indices = {len(self.sub_dataset_sizes) - 1}
                elif len(self.ref_list) == len(self.gt_list) and len(self.ref_list) > 0:
                    self.ref_dataset_indices = set(range(len(self.sub_dataset_sizes)))
                else:
                    self.ref_dataset_indices = set()

            # 预计算每个样本是否有 ref 的布尔数组 + gt_index -> ref_local_index 映射
            self.sample_has_ref_flag = [False] * len(self.gt_list)
            self.gt_to_ref_index = {}
            ref_offset = 0
            cumulative = 0
            for sub_idx, sub_size in enumerate(self.sub_dataset_sizes):
                if sub_idx in self.ref_dataset_indices:
                    for i in range(sub_size):
                        self.sample_has_ref_flag[cumulative + i] = True
                        self.gt_to_ref_index[cumulative + i] = ref_offset + i
                    ref_offset += sub_size
                cumulative += sub_size

            self.has_ref = len(self.gt_to_ref_index) > 0
            print(f"[Dataset] ref dataset indices: {sorted(self.ref_dataset_indices)}, "
                  f"{len(self.gt_to_ref_index)}/{len(self.gt_list)} samples have ref")

            # mask 同理（复用相同的 ref_dataset_indices，假设 ref 和 mask 始终绑定）
            self.sample_has_mask_flag = self.sample_has_ref_flag.copy()
            self.gt_to_mask_index = self.gt_to_ref_index.copy()
            self.has_mask = self.has_ref and len(self.hq_ori_mask_list) > 0
    ...
    def __getitem__(self, idx):
        if self.split == 'train':
            ...
            # 判断当前样本是否有真实 ref/mask
            sample_has_ref = self.sample_has_ref_flag[actual_idx]
            sample_has_mask = self.sample_has_mask_flag[actual_idx]

            use_boundary_dilation = True

            if sample_has_mask and len(self.hq_ori_mask_list) > 0 and len(self.ref_crop_mask_list) > 0:
                # mask 索引需要偏移，因为 mask_list 只包含有 mask 的子数据集
                mask_idx = self.gt_to_mask_index[actual_idx]

                if use_boundary_dilation:
                    lq_mask_full = Image.open(self.hq_ori_mask_list[mask_idx]).convert('L')
                    if self.use_resize_before_crop:
                        lq_mask_full = lq_mask_full.resize(gt_img.size, Image.Resampling.NEAREST)

                    # ===== LQ mask 膨胀 =====
                    import cv2
                    dilate_radius = random.randint(3, 6)
                    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * dilate_radius + 1, 2 * dilate_radius + 1))
                    mask_np = cv2.dilate(np.array(lq_mask_full), kernel, iterations=1)
                    lq_mask_full = Image.fromarray(mask_np)
                    from PIL import ImageFilter
                    lq_mask_full = lq_mask_full.filter(ImageFilter.GaussianBlur(radius=1))
                    # ===== 膨胀结束 =====
                    ...
                    if len(valid_coords) > 0 and random.random() < force_valid_prob:
                        center_y = int(valid_coords[:, 0].mean())
                        center_x = int(valid_coords[:, 1].mean())
                        offset_range = min(crop_h, crop_w) // 4
                        cy = center_y + random.randint(-offset_range, offset_range)
                        cx = center_x + random.randint(-offset_range, offset_range)
                    ...
                    # Ref mask 膨胀
                    ref_mask = Image.open(self.ref_crop_mask_list[mask_idx]).convert('L')
                    ref_dilate_radius = random.randint(5, 10)
                    ref_kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * ref_dilate_radius + 1, 2 * ref_dilate_radius + 1))
                    ref_mask_np = cv2.dilate(np.array(ref_mask), ref_kernel, iterations=1)
                    ref_mask = Image.fromarray(ref_mask_np).filter(ImageFilter.GaussianBlur(radius=1))

                    ref_w, ref_h = ref_mask.size
                    ref_pixels = ref_w * ref_h
                    if ref_pixels > self.ref_max_pixels:
                        scale = (self.ref_max_pixels / ref_pixels) ** 0.5
                        new_w = max(16, int(ref_w * scale) // 16 * 16)
                        new_h = max(16, int(ref_h * scale) // 16 * 16)
                    else:
                        new_w = max(16, ref_w // 16 * 16)
                        new_h = max(16, ref_h // 16 * 16)
                    ref_mask = ref_mask.resize((new_w, new_h), Image.Resampling.NEAREST)
