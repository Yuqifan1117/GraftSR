...
class QwenImageTransformerBlock(nn.Module):
    ...
    def _find_submodule_attr(self, module, attr_name, default=None):
        """从 module 及其所有子模块中查找第一个拥有 attr_name 的模块并返回其属性值。"""
    ...
    def _modulate(self, x, mod_params, index=None):
        ...
        elif mod_params.ndim == 3:
            # DualLoRA 展开后的 3D 情况，已与 x 序列长度对齐
            shift, scale, gate = mod_params.chunk(3, dim=-1)
    ...
    def forward(
        ...
        b, L_total, c = image.shape
        img_mod_attn, img_mod_mlp = self.img_mod(temb).chunk(2, dim=-1)  # [B, 3*dim] each
        # DualLoRA: img_mod 输出可能是 3D [B, num_slots, 3*dim]，需要展开到序列长度
        if img_mod_attn.ndim == 3:
            if img_mod_attn.shape[1] == 2:
                # DualLoRA: slot0=noise, slot1=condition
                noise_seq_len = self._find_submodule_attr(self.img_mod, 'noise_seq_len')
                if noise_seq_len is not None:
                    L_noise = noise_seq_len
                else:
                    num_cond = self._find_submodule_attr(self.img_mod, 'num_condition_images')
                    assert num_cond is not None, (
                        "DualLoRA requires noise_seq_len or num_condition_images on img_mod"
                    )
                    L_noise = L_total // (1 + num_cond)
                L_cond = L_total - L_noise
                img_mod_attn = torch.cat([
                    img_mod_attn[:, 0:1, :].expand(b, L_noise, -1),
                    img_mod_attn[:, 1:2, :].expand(b, L_cond, -1),
                ], dim=1)
                img_mod_mlp = torch.cat([
                    img_mod_mlp[:, 0:1, :].expand(b, L_noise, -1),
                    img_mod_mlp[:, 1:2, :].expand(b, L_cond, -1),
                ], dim=1)
...
class ConditionTypeEmbedding(nn.Module):
    """
    条件类型嵌入，与 BERT Segment Embedding 同构设计。
    为不同条件源（lq=0, ref=1）添加可学习的类型标记，
    使模型区分同一空间位置的不同来源，不干扰 RoPE 编码。
    零初始化保证训练初期等价于无 type embedding。
    """

    def __init__(self, dim: int = 3072, num_types: int = 2):
        super().__init__()
    ...
    def forward(self, tokens: torch.Tensor, type_id: int) -> torch.Tensor:
        return tokens + self.type_embed.weight[type_id].to(dtype=tokens.dtype)
